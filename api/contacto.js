...
export default async function handler(req,res){
  ...
  try{
    ...
    const subject=`Nueva consulta ThermomixTech - thermomixtech.es | ${nombre}`;
    const html=`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1B2230"><h2 style="color:#2F6E12">Nueva consulta desde ThermomixTech</h2><p><strong>Nombre:</strong> ${safe.nombre}</p><p><strong>Teléfono:</strong> ${safe.telefono}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Modelo Thermomix:</strong> ${safe.modelo}</p><p><strong>Descripción de la avería:</strong><br>${safe.mensaje}</p><hr><p style="font-size:12px;color:#66707F">Enviado desde el formulario de thermomixtech.es</p></div>`;
    const mime=[`From: ThermomixTech <${from}>`,`To: ${to}`,`Reply-To: ${email}`,`Subject: ${encodeHeader(subject)}`,'MIME-Version: 1.0','Content-Type: text/html; charset="UTF-8"','Content-Transfer-Encoding: 8bit','',html].join('\r\n');
